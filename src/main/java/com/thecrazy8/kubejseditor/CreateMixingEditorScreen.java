package com.thecrazy8.kubejseditor;

import net.minecraft.client.gui.components.Button;
import net.minecraft.client.gui.components.EditBox;
import net.minecraft.client.gui.screens.Screen;
import net.minecraft.network.chat.Component;

import java.io.File;
import java.io.FileWriter;
import java.io.IOException;

public class CreateMixingEditorScreen extends Screen {
    private final Screen parent;
    private EditBox outputItemBox;
    private EditBox ingredientsBox;
    private String heatLevel = "none"; // none, heated, superheated
    
    public CreateMixingEditorScreen(Screen parent) {
        super(Component.literal("Create: Mixing Recipe Editor"));
        this.parent = parent;
    }
    
    @Override
    protected void init() {
        super.init();
        
        int leftMargin = 20;
        int topMargin = 60;
        int fieldWidth = this.width - 40;
        int fieldHeight = 20;
        int spacing = 30;
        
        // Heat level buttons
        int btnWidth = 90;
        int btnSpacing = 95;
        this.addRenderableWidget(Button.builder(
            Component.literal("No Heat"),
            button -> heatLevel = "none"
        ).bounds(leftMargin, topMargin - 30, btnWidth, fieldHeight).build());
        
        this.addRenderableWidget(Button.builder(
            Component.literal("Heated"),
            button -> heatLevel = "heated"
        ).bounds(leftMargin + btnSpacing, topMargin - 30, btnWidth, fieldHeight).build());
        
        this.addRenderableWidget(Button.builder(
            Component.literal("Superheated"),
            button -> heatLevel = "superheated"
        ).bounds(leftMargin + btnSpacing * 2, topMargin - 30, btnWidth, fieldHeight).build());
        
        // Output Item
        this.addRenderableWidget(Button.builder(
            Component.literal("Output Item:"),
            button -> {}
        ).bounds(leftMargin, topMargin, 100, fieldHeight).build()).active = false;
        
        outputItemBox = new EditBox(this.font, leftMargin + 110, topMargin, fieldWidth - 110, fieldHeight, Component.literal("Output Item"));
        outputItemBox.setHint(Component.literal("e.g., create:chromatic_compound"));
        this.addRenderableWidget(outputItemBox);
        
        // Ingredients
        this.addRenderableWidget(Button.builder(
            Component.literal("Ingredients:"),
            button -> {}
        ).bounds(leftMargin, topMargin + spacing, 100, fieldHeight).build()).active = false;
        
        ingredientsBox = new EditBox(this.font, leftMargin + 110, topMargin + spacing, fieldWidth - 110, fieldHeight * 4, Component.literal("Ingredients"));
        ingredientsBox.setHint(Component.literal("One per line:\nminecraft:iron_ingot\n#forge:dusts/glowstone\nFluid.of('minecraft:water', 1000)"));
        this.addRenderableWidget(ingredientsBox);
        
        // Save button
        this.addRenderableWidget(Button.builder(
            Component.literal("Generate & Download Script"),
            button -> generateAndDownloadScript()
        ).bounds(this.width / 2 - 100, this.height - 50, 200, 20).build());
        
        // Back button
        this.addRenderableWidget(Button.builder(
            Component.literal("Back"),
            button -> this.minecraft.setScreen(parent)
        ).bounds(this.width / 2 - 100, this.height - 28, 200, 20).build());
    }
    
    private void generateAndDownloadScript() {
        try {
            String outputItem = outputItemBox.getValue().trim();
            String ingredients = ingredientsBox.getValue().trim();
            
            if (outputItem.isEmpty() || ingredients.isEmpty()) {
                return;
            }
            
            StringBuilder script = new StringBuilder();
            script.append("// Create: Mixing Recipe\n");
            script.append("// Generated by KubeJS GUI Editor\n\n");
            script.append("ServerEvents.recipes(event => {\n");
            script.append("    event.recipes.createMixing('").append(outputItem).append("', [\n");
            
            String[] lines = ingredients.split("\n");
            for (int i = 0; i < lines.length; i++) {
                String line = lines[i].trim();
                if (!line.isEmpty()) {
                    if (line.startsWith("Fluid.of")) {
                        script.append("        ").append(line);
                    } else {
                        script.append("        '").append(line).append("'");
                    }
                    if (i < lines.length - 1) script.append(",");
                    script.append("\n");
                }
            }
            
            script.append("    ])");
            
            if (heatLevel.equals("heated")) {
                script.append(".heated()");
            } else if (heatLevel.equals("superheated")) {
                script.append(".superheated()");
            }
            
            script.append(";\n");
            script.append("});\n");
            
            File minecraftDir = this.minecraft.gameDirectory;
            File kubejsDir = new File(minecraftDir, "kubejs/server_scripts");
            kubejsDir.mkdirs();
            
            File scriptFile = new File(kubejsDir, "create_mixing_custom.js");
            try (FileWriter writer = new FileWriter(scriptFile)) {
                writer.write(script.toString());
            }
            
            this.minecraft.player.sendSystemMessage(Component.literal("§aScript saved to: " + scriptFile.getAbsolutePath()));
            
        } catch (IOException e) {
            this.minecraft.player.sendSystemMessage(Component.literal("§cError saving script: " + e.getMessage()));
        }
    }
    
    @Override
    public void render(net.minecraft.client.gui.GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        this.renderBackground(guiGraphics, mouseX, mouseY, partialTick);
        super.render(guiGraphics, mouseX, mouseY, partialTick);
        guiGraphics.drawCenteredString(this.font, this.title, this.width / 2, 15, 0xFFFFFF);
        
        guiGraphics.drawString(this.font, "Heat Level: " + heatLevel, 20, 45, 0xFFFF00);
        guiGraphics.drawString(this.font, "For fluids use: Fluid.of('modid:fluid', amount)", 20, this.height - 65, 0xAAAAAA);
    }
    
    @Override
    public boolean isPauseScreen() {
        return false;
    }
}
